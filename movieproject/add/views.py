...
def addmovie(request,user_id):
	...
	if request.method == 'POST':
		...
		movie=Movie(name=name,image=image,trailer=trailer,banner=banner,desc=desc,category=category,relea_date=date,user=user)
		movie.save()
		return redirect('/')
	return render(request,'add_movie.html',{'user':user,'category':category})
